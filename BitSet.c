...
#define NUMBITS (sizeof(bitset_t) * 8) // 64
...
/**
 * Allocate, initialize and return a new (empty) BitSet with bits equals to value.
 */
BitSet new_BitSetFromValue(unsigned long long int value) {
	BitSet this = (BitSet)malloc(sizeof(struct BitSet));
	this->bits = value;
	return this;
}

unsigned long long int Bitset_toInt(BitSet this) {
	return this->bits;
}
